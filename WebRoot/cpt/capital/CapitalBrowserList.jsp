...
//zhb 修改资产资料浏览按钮，增加分部查看权限限制 2013-11-12
//查询排除流程
String pcSql="select count(*) from formtable_main_82 where ',' + CAST(pcr AS varchar) + ',' like '%,' + '"+user.getUID()+"' + ',%'";
rs.execute(pcSql);
int pcmode=0;
if(rs.next()){
	pcmode=rs.getInt(1);
}
if(user.getUID()!=1&&isCapital.equals("0")&&pcmode<1){
	sqlwhere+=" and t1.blongsubcompany = '"+user.getUserSubCompany1()+"'";
}
//new weaver.general.BaseBean().writeLog(sqlwhere);
